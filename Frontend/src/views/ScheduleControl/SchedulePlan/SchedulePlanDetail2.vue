  ...
  export default {
    ...
    methods: {
      getData () {
        ...
        api. getSchedulePlanFile2(this.id)
          .then(data => {
            this.name=data.name
            this.time=data.time
            this.fileList = data.audit
      ...
      handleChange (info) {
        if (info.file.status === 'done') {
          ...
          api. createSchedulePlanFile2(this.id, this.file)
            .then(data => {
      ...
      handleDelete (record) {
        api.deleteSchedulePlanFile2(record.id)
          .then(data => {
