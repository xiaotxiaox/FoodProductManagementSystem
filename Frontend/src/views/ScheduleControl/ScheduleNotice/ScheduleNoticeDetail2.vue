  ...
  export default {
    ...
    methods: {
      getData () {
        ...
        api. getScheduleNoticeFile2(this.id)
          .then(data => {
            this.id=data.id
            this.part=data.part
            this.give_time=data.give_time
            this.reply_time=data.reply_time
            this.fileList = data.reply_letter
      ...
      handleChange (info) {
        if (info.file.status === 'done') {
          ...
          api. createScheduleNoticeFile2(this.id, this.file)
            .then(data => {
      ...
      handleDelete (record) {
        api.deleteScheduleNoticeFile2(record.id)
          .then(data => {
