  ...
  export default {
    ...
    methods: {
      getData () {
        ...
        api. getInvestChangeFile1(this.id)
          .then(data => {
            this.name=data.name
            this.out_list=data.out_list
            this.contract_price=data.contract_price
            this.change_price=data.change_price
            this.price_before=data.price_before
            this.price_after=data.price_after
            this.number_before=data.number_before
             this.number_after=data.number_after
            this.fileList = data.accessory
      ...
      handleChange (info) {
        if (info.file.status === 'done') {
          ...
          api. createInvestChangeFile1(this.id, this.file)
            .then(data => {
      ...
      handleDelete (record) {
        api.deleteInvestChangeFile1(record.id)
          .then(data => {
