...
<script>
  ...
  export default {
    name: 'ElseDetailModal',
    props: {
      record: Object,
      visible: Boolean,
      type: String,
      else_detail_id: Number
    },
    data() {
      return {
        layout: {
    ...
    methods: {
      getData() {
        if (this.isEdit) {
          api.getElseDetailMatter(this.record.matter_id)
            .then(data => {
              this.matter = data
            })
        }
      ...
      handleOk() {
        this.form.validateFields((error, data) => {
          if (!error) {
            if (this.isEdit) {
              api.updateElseDetailMatter(this.record.matter_id, data)
                .then(data => {
                  this.$notification.success({message: '成功', description: '更新成功', key: 'SUCCESS'})
                  this.$emit('close')
                })
            } else {
              api.createElseDetailMatter(this.else_detail_id, data)
                .then(data => {
